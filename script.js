...
/* ============================================================
   MAGIC MALL MODULAR INTEGRATION - MERGE & LAYER LOGIC
   ============================================================ */

/**
 * Entry point for merging a new file into the current animData
 */
async function handleMergeFile(file) {
    if (!animData) {
        alert("Please load an animation first before merging.");
        return;
    }
    ...
    reader.onload = async (e) => {
        try {
            ...
            performLottieMerge(incoming, file.name);

            // 3. Sanitize the final result to ensure Telegram/Lottie compatibility
            sanitizeLottieData(animData);

            // 4. Refresh the UI and Preview
            // Uses your existing update functions
            if (typeof updateEditorUI === 'function') updateEditorUI();
            if (typeof playAnimation === 'function') playAnimation(animData);
            
            console.log(`Successfully merged: ${file.name}`);
        } catch (err) {
            console.error("Merge error:", err);
            alert("Merge failed. Please check if the file is a valid Lottie JSON.");
        }
    };

    if (file.name.endsWith('.tgs')) reader.readAsArrayBuffer(file);
    else reader.readAsText(file);
}

/**
 * Deep merge logic to combine two Lottie files
 */
function performLottieMerge(incoming, fileName) {
    // Determine the highest existing index to prevent ID collisions
    let maxInd = 0;
    animData.layers.forEach(l => { if (l.ind > maxInd) maxInd = l.ind; });
    const indOffset = maxInd + 1;
    const assetPrefix = "m" + Date.now().toString(36) + "_";

    // Create a Null layer to act as a "Folder" for the merged items
    ...
    const incomingGroup = {
        ...
        ks: {
            o: { a: 0, k: 100 },
            r: { a: 0, k: 0 },
            p: { a: 0, k: [animData.w / 2, animData.h / 2, 0] },
            a: { a: 0, k: [0, 0, 0] },
            s: { a: 0, k: [100, 100, 100] }
        }
    };

    // Clone and process layers from the incoming file
    ...
    clonedLayers.forEach(layer => {
        ...
        // Reparent layers to the new Group Null
        if (layer.parent !== undefined) {
            layer.parent += indOffset + 1;
        } else {
            layer.parent = indOffset; 
        }

        // Handle Asset References (Images/Precomps)
        if ((layer.ty === 0 || layer.ty === 2) && layer.refId) {
            layer.refId = assetPrefix + layer.refId;
        }
    });

    // Merge Assets (Images/Fonts/Precomps)
    if (incoming.assets) {
        if (!animData.assets) animData.assets = [];
        incoming.assets.forEach(asset => {
            const clonedAsset = JSON.parse(JSON.stringify(asset));
            clonedAsset.id = assetPrefix + clonedAsset.id;
            animData.assets.push(clonedAsset);
        });
    }

    // Expand composition time if merged animation is longer
    if (incoming.op > animData.op) animData.op = incoming.op;

    // Push the new group and layers into the existing array
    animData.layers.push(incomingGroup, ...clonedLayers);
}

/**
 * Sanitizer to ensure the JSON structure remains valid for Export
 */
function sanitizeLottieData(data) {
    if (!data.v) data.v = '5.5.2';
    if (!data.fr) data.fr = 60;
    if (!data.w) data.w = 512;
    if (!data.h) data.h = 512;
    if (!data.assets) data.assets = [];
    
    data.layers.forEach((layer, index) => {
        if (!layer.nm) layer.nm = 'Layer ' + (layer.ind || index);
        if (layer.ty === 4 && !layer.shapes) layer.shapes = [];
        // Ensure every layer has transform data
        if (!layer.ks) {
            layer.ks = {
                p: {k:[0,0,0]}, r: {k:0}, s: {k:[100,100,100]}, o: {k:100}, a: {k:[0,0,0]}
            };
        }
    });
}
